...
1. **フロントエンド**: UI表示制御（権限のないボタンを非表示/無効化）
2. **バックエンド**: API呼び出し時の権限検証（セキュリティの最終防衛線）
3. **ルーティング**: 画面アクセス時の権限確認（取引確定画面のみ管理者限定、未認可の場合は403エラー）
...
### プロパティ 10: 口座開設時の初期残高制約

*任意の*口座開設において、初期残高は1円以上でなければならず、1円未満の場合は開設が拒否されること
**検証対象: 要件 3.2**
...
| 取引確定         | `/transactions/confirmation`      | `TransactionConfirmation` | 必要     | 管理者のみ | 取引の最終確定（管理者のみ） |
...
| ユーザー管理     | `/settings/users`                 | `UserManagement`          | 必要     | 管理者のみ | ユーザー管理（管理者のみ）   |
| システム設定     | `/settings/system`                | `SystemConfig`            | 必要     | 管理者のみ | システム設定（管理者のみ）   |
...
- **権限チェック**: 各操作実行前にユーザー役割に基づく権限を確認
- **セッション管理**: セッション期限切れ時の自動ログイン画面遷移
- **リダイレクト**: ログイン成功後の元画面への自動遷移
- **エラー表示**: 権限不足時の403エラーページ表示

#### 権限別アクセス制御

**一般行員がアクセスできない画面**:

- `/transactions/confirmation` - 取引確定（管理者のみ）
- `/settings/users` - ユーザー管理（管理者のみ）
- `/settings/system` - システム設定（管理者のみ）
