...
| TXN009 | 操作       | ボタン        | 承認・却下ボタン（自己作成時は非活性） | -                                                         |
...
| ------ | ------------ | -------------- | ---- | -------------------------------- | ------ | ------ |
| TRV011 | 詳細表示     | モーダル       | -    | -                                | -      | -      |
| TRV012 | 承認ボタン   | ボタン         | -    | 作成者と同一ユーザの場合は非活性 | -      | -      |
| TRV013 | 却下ボタン   | ボタン         | -    | 作成者と同一ユーザの場合は非活性 | -      | -      |
...
| ボタン名   | 非活性条件                     | 表示メッセージ           | 根拠                  |
| ---------- | ------------------------------ | ------------------------ | --------------------- |
| 承認ボタン | 取引作成者と検証者が同一ユーザ | 「検証不可（自己作成）」 | 要件5.6: 自己検証防止 |
| 却下ボタン | 取引作成者と検証者が同一ユーザ | 「検証不可（自己作成）」 | 要件5.6: 自己検証防止 |

### UI表示制御

| 条件                         | 制御内容                   | 表示内容                       |
| ---------------------------- | -------------------------- | ------------------------------ |
| 作成者と検証者が同一ユーザ   | 承認・却下ボタンを非活性化 | 「検証不可（自己作成）」       |
| 作成者と検証者が異なるユーザ | 承認・却下ボタンを活性化   | 「状態管理」ボタン             |
| 検証待ち取引が存在しない     | 空状態画面を表示           | 「検証待ちの取引はありません」 |
